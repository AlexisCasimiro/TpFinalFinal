$(document).ready(function() {
    $('#agregar-producto').submit(function(event) {
        ...
        // Si las verificaciones son exitosas, enviar el formulario
        if(nombreValid && detalleValid && cantStockValid && precioValid && imagenInputValid){
            var formulario=document.getElementById('agregar-producto');
            // Obtener los datos del formulario
            var formData = new FormData(formulario); 
            // Enviar los datos del formulario utilizando AJAX
            $.ajax({
                type: 'POST',
                url: '../Accion/aniadirProducto.php',
                data: formData,
                processData: false,
                contentType: false,
            ...
            //document.getElementById('agregar-producto').submit();
        }
    });  
  });